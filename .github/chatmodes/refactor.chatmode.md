...
tools: ['codebase', 'editFiles', 'runCommands', 'search', 'searchResults', 'usages', 'problems', 'findTestFiles', 'terminalLastCommand', 'changes', 'fetch']
---
...
- **Name the smell**: what's making this code hard to work with right now? (See `/refactor` for the catalog.)
...
When the user is asking *whether* to refactor — not asking you to refactor — switch to `/challenge` or `/options` mode of thinking and help them decide.
...
- Pivot to `/build` if the user actually needs new behavior, not restructuring.
- Pivot to `/diagnose` if the refactor exposes a bug.
- Use `/refactor` (the prompt) for the full Fowler + Beck + Feathers catalog.
- Use `/expert-panel` when there's a real design tension worth surfacing.
