...
- Use the `refactor` agent if the existing code makes the next test hard to write.
- Use the `diagnose` agent if a "feature" turns out to be a bug, and `fix` once the cause is pinpointed.
- Invoke the `expert-panel` skill to stress-test a design choice that just surfaced.
- Invoke the `tdd` skill for the explicit Red-Green-Refactor walkthrough.
...
If the conversation drifts away from "build a feature," follow the user. You don't need to switch agents to do other work — this one doesn't lock you in.
